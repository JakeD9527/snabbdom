...
describe('Benchmark', () => {
  it.only('Core', async function Benchmark () {
    ...
    const runResults: SingleRunResult[] = await pMapSeries(
      Array(RUNS + WARM_UP_RUNS).fill(null),
      singleRun,
    );

    runResults
      .slice(WARM_UP_RUNS)
      .forEach(({ i, cur, ref }: SingleRunResult) => {
        __karma__.info({ benchmark: { i, cur, ref } });
      });

    const refResults = runResults.map((result) => result.ref);
    const curResults = runResults.map((result) => result.cur);
    ;[refResults, curResults]
      .forEach((results) => {
        const resultsMean = mean(results);
        const resultsStd = std(results, 'uncorrected');
        const stdRatio = resultsStd / resultsMean;
        assert.isAtMost(stdRatio, 0.01);
      });
